...
<script src="[{ $oViewConf->getResourceUrl() }]packstation.js" type="text/javascript" ></script>
    ...
    <form action="[{ $oViewConf->getSelfActionLink() }]" name="order" method="post">
      <div class="account">
          ...
          <tr>
			...
			<td>
              <input id="packstr" type="text" size="28" maxlength="255" onchange="if(this.value == ' '){this.form.chkpack.checked = false;}" name="deladr[oxaddress__oxstreet]" value="[{if $delivadr->oxaddress__oxstreet->value }][{ $delivadr->oxaddress__oxstreet->value }][{elseif $deladr.oxaddress__oxstreet }][{ $deladr.oxaddress__oxstreet }][{/if }]">
          ...
          <tr>
            ...
            <td>
              <select id="countryoption" [{if $delivadr->oxaddress__oxstreet->value == 'Packstation'}] disabled style="display: none;" [{else}] style="display: inline;"[{/if}] name="deladr[oxaddress__oxcountryid]">
                <option value="">-</option>
             	[{foreach from=$oView->getCountryList() item=country key=country_id }]
